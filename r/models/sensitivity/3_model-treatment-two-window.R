...
config_name <- "ensemble-bern_90_c-0_90_r-0_90_pb_two_window"
...
jags_model_file <- file.path(data_intermediate, "logistic_mixed_pb_two_window.jags")
...
data_full <- data_full %>%
  mutate(
    ...
    pb_10_20 = ifelse(!is.na(year_burnt) & year_burnt >= 2001 & year_burnt < 2011, 1, 0)
  )
...
model {
  for (i in 1:N) {
    ...
    mu[i] <- b_0
          ...
          + b_ladder    * ladder[i]
          + b_rumple    * rumple[i]
          + b_cdensity  * cdensity[i]
          + b_pb_recent * pb_recent[i]
          + b_pb_10_20  * pb_10_20[i]
          + b_upslope   * upslope[i]
          + b_solar     * solar[i]
          + b_slope     * slope[i]
          + b_tpi       * tpi[i]
  ...
  b_0         ~ dnorm(0, 0.01)
  b_zq95      ~ dnorm(0, 0.01)
  b_ladder    ~ dnorm(0, 0.01)
  b_rumple    ~ dnorm(0, 0.01)
  b_cdensity  ~ dnorm(0, 0.01)
  b_pb_recent ~ dnorm(0, 0.01)
  b_pb_10_20  ~ dnorm(0, 0.01)
...
res <- foreach::foreach(
  ...
  if (!file.exists(out_rds)) {
    ...
    data_list <- list(
      N = nrow(df_sub),
      zq95 = df_sub$zq95,
      ladder = df_sub$ladder1,
      rumple = df_sub$rumple_0_90,
      cdensity = df_sub$cdensity_0_90,
      pb_recent = df_sub$pb_recent,
      pb_10_20 = df_sub$pb_10_20,
